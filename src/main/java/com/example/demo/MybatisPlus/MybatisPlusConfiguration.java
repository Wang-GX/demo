...
public class MybatisPlusConfiguration {
     ...
     *     根据加锁对象的不同分为行锁和表锁（Mysql，只有InnoDB存储引擎才有行锁，MyISAM只有表锁），以及加锁机制不同分为共享锁和排他锁。
     *      ①行锁和表锁：
     ...
     * 线程开始执行阶段就到数据库中查询并记录下数据当前的版本号，更新时从数据库中查询最新的版本号并与记录值进行对比，如果相同则表示在此期间该数据没有被修改过(即在当前线程操作这条数据期间没有其他线程操作过这条数据，即不存在并发)，才会去更新，否则就不更新。
     * 也可以理解为，提交版本(oldVersion + 1)必须大于当前版本才能执行更新
