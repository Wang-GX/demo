 ...
 */
@Configuration
@ConfigurationProperties(prefix = "author")//指定前缀批量注入
/**
 * 注入配置文件中的配置有两种方式：
...
public class DefaultConfig {
    ...
    //通过@Value注入author.height2属性值，然后调用set方法注入author.height属性值，所以最终的值为author.height的属性值[通过debug观察]
...
/*    public String getHeight() {
    ...
    public User getUser(){
        ...
        user.setAuthorName(name);
        user.setAuthorHeight(height);
        return user;
    }
}
